...
import UserEdit from './UserEdit.jsx'; // importa tu componente de edición
    ...
    <CustomMenuMobile>
        ...
        <Box sx={{ display: 'flex',  justifyContent:'end', alignItems: 'center', gap:3 }}>
          ...
          <Button onClick={() => setShowSearchExpanded(false)}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '30vw' }}>
              <CustomTextField
                label={t('searchBar')}
                value={searchTerm}
                onChange={e => setSearchTerm(e.target.value)}
              ...
              <Button onClick={handleSearchClick}>
                <FontAwesomeIcon style={{ fontSize: 14, color: '#3e4a4c' }} icon={faMagnifyingGlass} />
              </Button>
            </Box>
          )}
            <CustomLink href="/lists" underline="hover"><FontAwesomeIcon style={{ fontSize: 12, color: '#3e4a4c' }} icon={faListUl} /></CustomLink>
            <CustomLink href="/tops" underline="hover" ><FontAwesomeIcon style={{ fontSize: 12, color: '#3e4a4c' }} icon={faMedal} /></CustomLink>
            
            {role === 'admin' && <Link sx={{color: 'red'}} href="/admin" underline="hover">Admin</Link>}
            {/* Avatar con onClick para abrir el modal */}
            <Avatar
              sx={{width: '60px', height: '60px', cursor: 'pointer', marginTop:'5px', marginBottom:'5px'}}
              src={
                currentUser && currentUser.profilePic
              ...
              onClick={() => navigate('/profile')}
            />
            <Button  onClick={logout} sx={{ color: 'gray', borderColor: 'gray', width: '30px', fontSize: 10 }}><FontAwesomeIcon style={{ fontSize: 24, color: '#3e4a4c' }} icon={faXmark} /></Button>
        </Box> 
      </CustomMenuMobile>
    </Box>
  );
